...
int Application::GetCommand() {
	...
	cout << "\t   5 : Search the item by FileName" << endl;  // using binary search.
	cout << endl;
	cout << "\t*********Gallery Research*********" << endl;
	...
	cout << endl;
	cout << "\t********My Favorite Gallery********" << endl;
	cout << "\t   10 : Add a favorite item" << endl;
	cout << "\t   11 : Delete a favorite item" << endl;
	cout << "\t   12 : Display all favorite items" << endl;
	cout << "\t   13 : Display details of all items" << endl;
...
void Application::DeleteContents() {
	...
	int length = m_ContentsList.GetLength();
	int num = 0;
	int curIndex = m_ContentsList.GetNextItem(data); //curIndex = 0, data has the first record of the list.
...
void Application::AddFavoriteItem()
{
	...
	if (m_ContentsList.GetByBinarySearch(tmpItem)) {
		item.SetPeople(tmpItem.GetPeople());
...
// Print the details(information in masterlist) of the favorite list.
void Application::PrintFavoriteItemsDetails() {
	...
	while (curIndex1 < length1 && curIndex1 != -1)
	{
		for_print1.SetRecord(data1.GetFileName(), -1, 0, "000000", "000000", "000000");
		m_ContentsList.GetByBinarySearch(for_print1);
		for_print1.DisplayRecordOnScreen();
	...
	cout << endl;
	m_FavoriteList2.ResetList(); //initialize the list iterator into -1.
	int length = m_FavoriteList2.GetLength();
	int curIndex = m_FavoriteList2.GetNextItem(data2);

	while (curIndex < length && curIndex != -1)
	{
		for_print2.SetRecord(data2.GetFileName(), -1, 0, "000000", "000000", "000000");
		m_ContentsList.GetByBinarySearch(for_print2);
		for_print2.DisplayRecordOnScreen();

		curIndex = m_FavoriteList2.GetNextItem(data2);
	}
}
...
void Application::FindNDisplayByEventAndPerson() {
	...
	if (m_EventList.Get(tempE) && m_PersonList.Get(tempP)) { 
		...
		for (int i = 0; i < tempE.getFileNameList().GetLength(); i++)
		{
				tempList.GetNextItem(tempFileName);				// move to the next item in the filenamelist.
		...
		if (BothList.GetLength()==0) { 
			...
			cout << endl;
			for (int i = 0; i < BothList.GetLength(); i++) {
				ItemType tempItemType;
...
// Consider the state of the gallery search and my favorite gallery after deletion in my gallery. 
// Change the expression 'Research' into -> 'Search' 
